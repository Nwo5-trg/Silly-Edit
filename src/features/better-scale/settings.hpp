...
namespace Settings::BetterScale {
    ...
    inline SillySetting<bool> newLockTexture{"New Lock\nTexture", "Better Scale", true, SettingReload::Editor};
    inline SillySetting<bool> switchModeButton{"Switch Mode\nButton", "Better Scale", true, SettingReload::Editor};
    inline SillySetting<float> controlSize{"Control Size", "Better Scale", 1.0f, {0.1f, std::nullopt}, "multiplies scale control size"};
    inline SillySetting<bool> lockControlSize{"Lock Control\nSize", "Better Scale", false, "treats control size as a static scale that doesnt change with zoom"};
